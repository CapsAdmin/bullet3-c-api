...
extern "C"
{
	...
	// update
	EXPORT void bulletStepSimulation(float time_step, int sub_steps, float fixed_time_step)
	{
		world->stepSimulation(time_step,  sub_steps, fixed_time_step);
	}
	...
	EXPORT int bulletRayCast(float from_x, float from_y, float from_z, float to_x, float to_y, float to_z, bullet_raycast_result *out)
	{
		...
		if (hit)
		{
			...
			res.body = (btRigidBody *)cb.m_collisionObject;
...
extern "C"
{
	...
	EXPORT btRigidBody *bulletCreateRigidBodyConvexHull(float mass, float *matrix, btScalar *mesh, int num_points)
	{
		btCollisionShape *shape = new btConvexHullShape(mesh, num_points);

		return create_body(mass, matrix, shape);
	}

	EXPORT btRigidBody *bulletCreateRigidBodyConvexTriangleMesh(float mass, float *matrix, btTriangleIndexVertexArray *mesh)
	{
	...
	EXPORT btRigidBody *bulletCreateRigidBodyTriangleMesh(float mass, float *matrix, btTriangleIndexVertexArray *mesh, int quantized_aabb_compression)
	{
	...
	// wake
	EXPORT void bulletRigidBodyGetLinearSleepingThreshold(btRigidBody *body, float *out)
	{
		out[0] = body->getLinearSleepingThreshold();
	}
	EXPORT void bulletRigidBodySetLinearSleepingThreshold(btRigidBody *body, float threshold)
	{
		body->setSleepingThresholds(threshold, body->getAngularSleepingThreshold());
	}

	EXPORT void bulletRigidBodyGetAngularSleepingThreshold(btRigidBody *body, float *out)
	{
		out[0] = body->getAngularSleepingThreshold();
	}
	EXPORT void bulletRigidBodySetAngularSleepingThreshold(btRigidBody *body, float threshold)
	{
		body->setSleepingThresholds(body->getLinearSleepingThreshold(), threshold);
	}
}
